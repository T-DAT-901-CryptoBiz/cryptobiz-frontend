<template>
  <div class="min-h-screen bg-[#1A1B1F] flex items-center justify-center px-4 py-12">
    <div class="w-full max-w-md">
      <div class="rounded-xl bg-[#26292E] border border-white/10 overflow-hidden shadow-2xl">
        <div class="px-8 py-8">
          <!-- Header avec logo et titre -->
          <div class="mb-8">
            <div class="flex items-center gap-2 mb-4">
              <div class="h-8 w-8 rounded-lg bg-white grid place-items-center">
                <img src="/cryptobiz-logo.png" alt="CryptoBiz" class="h-6 w-6" />
              </div>
              <span class="text-2xl font-bold text-white">CRYPTOBIZ</span>
            </div>
            <h1 class="text-3xl font-bold text-white">Register</h1>
          </div>

          <form @submit.prevent="handleRegister" class="space-y-5">
            <div>
              <label class="block text-sm font-medium text-white mb-2">Name</label>
              <input
                v-model="form.name"
                type="text"
                placeholder="Your full name"
                required
                :disabled="loading"
                class="w-full px-4 py-3 rounded-lg bg-[#33363A] border border-white/10 text-white placeholder:text-white/40 focus:outline-none focus:border-white/50 transition-colors disabled:opacity-50"
              />
            </div>

            <div>
              <label class="block text-sm font-medium text-white mb-2">Email/Phone number</label>
              <input
                v-model="form.email"
                type="email"
                placeholder="Email/Phone number (without country code)"
                required
                :disabled="loading"
                class="w-full px-4 py-3 rounded-lg bg-[#33363A] border border-white/10 text-white placeholder:text-white/40 focus:outline-none focus:border-white/50 transition-colors disabled:opacity-50"
              />
            </div>

            <div>
              <label class="block text-sm font-medium text-white mb-2">Password</label>
              <input
                v-model="form.password"
                type="password"
                placeholder="Minimum 6 characters"
                required
                :disabled="loading"
                class="w-full px-4 py-3 rounded-lg bg-[#33363A] border border-white/10 text-white placeholder:text-white/40 focus:outline-none focus:border-white/50 transition-colors disabled:opacity-50"
              />
              <p class="mt-1.5 text-xs text-white/70">
                Password must contain at least 6 characters
              </p>
            </div>

            <div
              v-if="error"
              class="px-4 py-3 rounded-lg bg-red-500/10 border border-red-500/20 text-white text-sm"
            >
              {{ error }}
            </div>

            <button
              type="submit"
              :disabled="loading"
              class="w-full py-3 rounded-lg bg-white text-black font-semibold hover:bg-white/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
            >
              <span v-if="!loading">Create my account</span>
              <span v-else class="flex items-center gap-2">
                <Icon name="lucide:loader-2" class="w-4 h-4 animate-spin" />
                Loading...
              </span>
            </button>
          </form>

          <!-- Séparateur -->
          <div class="relative my-6">
            <div class="absolute inset-0 flex items-center">
              <div class="w-full border-t border-white/10"></div>
            </div>
            <div class="relative flex justify-center">
              <span class="px-3 bg-[#26292E] text-white text-sm">or</span>
            </div>
          </div>

          <!-- Boutons de connexion alternatifs -->
          <div class="space-y-3">
            <button
              type="button"
              :disabled="loading"
              @click="handleGoogleLogin"
              class="w-full px-4 py-3 rounded-lg bg-[#26292E] border border-white/10 text-white font-medium hover:bg-[#2E3238] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3"
            >
              <svg class="w-5 h-5" viewBox="0 0 24 24">
                <path
                  fill="#4285F4"
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                />
                <path
                  fill="#34A853"
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                />
                <path
                  fill="#FBBC05"
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                />
                <path
                  fill="#EA4335"
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                />
              </svg>
              <span>Continue with Google</span>
            </button>
          </div>

          <!-- Lien de connexion -->
          <div class="mt-8 text-center">
            <NuxtLink
              to="/login"
              class="text-white hover:text-white/80 font-medium transition-colors text-sm"
            >
              Already have an account? Log in
            </NuxtLink>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
definePageMeta({
  layout: false,
})

useHead({
  title: 'Register - CryptoBiz',
})

const { register, loginWithGoogle } = useAuth()
const router = useRouter()

const form = ref({
  name: '',
  email: '',
  password: '',
})

const loading = ref(false)
const error = ref<string | null>(null)

async function handleRegister() {
  if (!form.value.name || !form.value.email || !form.value.password) {
    error.value = 'Please fill in all fields'
    return
  }

  if (form.value.password.length < 6) {
    error.value = 'Password must contain at least 6 characters'
    return
  }

  loading.value = true
  error.value = null

  try {
    await register(form.value.name, form.value.email, form.value.password)
    await router.push('/')
  } catch (err: unknown) {
    error.value =
      err && typeof err === 'object' && 'data' in err
        ? String((err.data as { message?: string }).message || 'Registration error')
        : 'Registration error'
  } finally {
    loading.value = false
  }
}

function handleGoogleLogin() {
  loginWithGoogle()
}
</script>
